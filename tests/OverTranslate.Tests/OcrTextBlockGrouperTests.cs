...
public class OcrTextBlockGrouperTests
{
    ...
    /// <summary>
    /// A third line joins on the leading the first two already established, past the fixed limit.
    /// </summary>
    /// <remarks>
    /// The first pair is set at 1.20 exactly, which the fixed limit admits. The third line sits
    /// 1.26 under the second — past that limit, inside the group's own leading plus the noise
    /// allowance. A fixed number cannot take it; the group's own can.
    /// </remarks>
    [Fact]
    public void AThirdLine_JoinsOnTheLeadingTheGroupHasAlreadyEstablished()
    {
        var first = Line("CHANCELLOR ARRANGED THE REWARDS", x: 100, y: 0, width: 500, height: 40);
        var second = Line("ACCORDING TO THE EXPLORERS MERITS", x: 100, y: 48, width: 500, height: 40);
        var third = Line("AND THE ORDER IN WHICH THEY FELL", x: 100, y: 98, width: 500, height: 40);

        var decisions = new List<OcrTextBlockGrouper.NextLineDecision>();
        var grouped = OcrTextBlockGrouper.Group(
            [first, second, third], GroupingProfile.Standard, decisions);

        var verdicts = decisions.Where(decision => decision.Kind == "next").ToList();
        Assert.Equal(2, verdicts.Count);
        Assert.Equal(1.20, verdicts[0].SolidBar, precision: 2);
        Assert.Equal(1.32, verdicts[1].SolidBar, precision: 2);
        Assert.True(verdicts[1].LineAdvance > 1.20, "the third line is past the fixed limit");
        Assert.Single(grouped);
    }

    /// <summary>
    /// A group that got together on something other than its setting cannot widen its own limit.
    /// </summary>
    /// <remarks>
    /// The first pair here joins on the comma, at a leading of 1.30 — looser than anything this
    /// rule would take as set solid. Reading that back as "this group is set at 1.30" would let one
    /// join bought with punctuation pay for a second bought with nothing, and the group would walk
    /// down the page a line at a time. So a leading looser than the fixed limit is not a leading
    /// this rule believes, and the third line is judged against the fixed limit like any other.
    /// </remarks>
    [Fact]
    public void AGroupJoinedOnPunctuation_DoesNotWidenItsLimitToItsOwnLooseLeading()
    {
        var first = Line("THE FIRST LINE ENDS WITH A COMMA,", x: 100, y: 0, width: 500, height: 40);
        var second = Line("THE SECOND LINE CARRIES IT ONWARD", x: 100, y: 52, width: 500, height: 40);
        var third = Line("THE THIRD LINE IS A SEPARATE ROW", x: 100, y: 104, width: 500, height: 40);
        ...
        var verdicts = decisions.Where(decision => decision.Kind == "next").ToList();
        Assert.Equal("punctuation", verdicts[0].Rule);
        Assert.Equal(1.30, verdicts[0].LineAdvance, precision: 2);

        // The bar stayed where it was, so the third line is refused.
        Assert.Equal(1.20, verdicts[1].SolidBar, precision: 2);
        Assert.NotEqual("set solid", verdicts[1].Rule);
        Assert.Equal(2, grouped.Count);
    }

    /// <summary>
    /// A group that has established nothing is held to the fixed limit.
    /// </summary>
    /// <remarks>
    /// This is why a list cannot exploit the rule above: a list's entries never join, so a group
    /// there is always one line long and never has a leading of its own to widen with.
    /// </remarks>
    [Fact]
    public void ASingleLine_HasNoLeadingOfItsOwn_AndIsHeldToTheFixedLimit()
    {
        var previous = Line("Automatically navigate to coffers", x: 100, y: 10, width: 500, height: 40);
        var current = Line("Prioritize opening coffers over cairns", x: 100, y: 60, width: 500, height: 40);

        var verdict = NextLineVerdict(previous, current);

        Assert.Equal(1.20, verdict.SolidBar, precision: 2);
        Assert.False(verdict.Joined);
    }
