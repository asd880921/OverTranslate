...
> 目前只建議使用 Microsoft、OpenAI、DeepL 進行翻譯 (延遲較低)。  
...
> 來源語言為自動偵測時沒有來源語言可代入，設定頁會隱藏 `{source_name}` 與 `{source_code}`；  
> 內建提示詞只使用語言名稱，語言代碼參數是留給需要「Japanese (ja)」這類寫法的模型自行編排。

> API 位址、模型名稱與提示詞留空時，欄位會以淡色顯示實際會套用的預設值。  
