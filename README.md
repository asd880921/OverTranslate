...
## 即時翻譯
> 快捷鍵（預設 `Ctrl + Alt + S`）可直接進入翻譯區塊選取模式，不需要先開啟主視窗；  
...
原文欄位可一鍵清除；來源語言為自動偵測時，因為無法確定要用哪種語音，原文朗讀會停用。
...
> 1.開啟翻譯視窗、即時翻譯區塊的快捷鍵可個別停用（截圖翻譯為核心功能，不提供停用）；停用後該列的輸入框與錄製按鈕會一併關閉，變更後立即生效，不需要重新啟動。  
> 2.若不同功能設定到相同的組合鍵，會依「截圖翻譯 → 開啟翻譯視窗 → 即時翻譯區塊」的順序保留，設定頁會標示實際失效的項目。
