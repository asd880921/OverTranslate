...
**遊戲介面**：適合文字分散於不同位置，或內容位置經常變動的遊戲畫面。
...
| 開啟翻譯視窗 | 呼叫主視窗的快捷鍵 (預設 `Ctrl + Alt + S`)；即時翻譯進行時則將浮動視窗移至最上層 |
| 來源語言 | 原文 (預設 Auto) |
...
> Log 僅會保留在本機，不會自動上傳，即使開啟 **應用紀錄** 紀錄更詳細的資訊，也只會在本地儲存，  
需由使用者主動提供 Log 給開發者協助確認。
> 
---
...
- **OpenAI**：需自備 OpenAI API 相容服務，可使用雲端服務或本地 LLM；本地使用方式可參考 [Ollama 安裝教學](OLLAMA_GUIDE.md)
