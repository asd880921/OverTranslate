...
- ⚡ **取詞翻譯** — 快捷鍵叫出小視窗，自動帶入選取的文字，看完移開滑鼠就消失
- ⚡ **取詞翻譯** — 快捷鍵叫出小視窗，自動帶入選取的文字，看完移開滑鼠就消失
...
## 取詞翻譯
> 快捷鍵（預設 `Ctrl + Alt + Q`）呼叫，沒有其他入口。

在任何地方選取文字後按下快捷鍵，選取的內容會自動帶進一個小視窗翻譯；沒有選取文字時，
視窗會以空白輸入框出現，直接輸入即可。小視窗出現在滑鼠附近，**滑鼠離開一秒後會自動關閉**。

- 拖曳可移動位置，移動後會自動釘選；釘選後就不會自動關閉。
- 齒輪會在同一個小視窗內切換設定，可切換翻譯服務與是否自動朗讀譯文。
- 來源語言、目標語言與翻譯服務與 **截圖翻譯**、**文字翻譯** 共用同一組設定。
- 同時只會有一個小視窗；重複按快捷鍵是把新選取的文字換進去，不會再開一個。
...
| 取詞翻譯 (快捷鍵) | 將選取的文字帶進小視窗翻譯 (預設 `Ctrl + Alt + Q`) |
| 取詞翻譯 (快捷鍵) | 將選取的文字帶進小視窗翻譯 (預設 `Ctrl + Alt + Q`) |
