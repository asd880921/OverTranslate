...
範囲を選んだあと、ツールバーからは次の操作もできます。

* **テキストをコピー / 訳文をコピー**：認識した原文または訳文をそのままクリップボードにコピーします
* **マークアップ**：ペン・蛍光ペン・消しゴムで、キャプチャの上に直接書き込めます
* **スクリーンショット**：選んだ範囲をクリップボードにコピーし、同時に画像ファイルとして保存もできます
...
> ログと診断情報はお使いの PC にのみ保存され、自動で送信されることはありません。問題を報告するときは **診断情報を書き出す** を押し、続いて **アップロード** を選んでください。ファイルは 30 日後に自動で削除されます。
