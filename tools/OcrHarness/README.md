...
判讀時務必**多試幾種成功判準**。用「字數等於該圖最大值」這種最嚴的判準，會量出一條看起來
單調上升到 40–50px 的漂亮曲線；把判準放寬到 0.85 之後，同一批資料在 40px 以上反而下降。
跨兩個 corpus、四種容差都成立的只有一件事：**glyphDetectorPx 低於約 15px 會崩潰，超過約 20–25px
之後幾乎沒有差別**。那是一條地板，不是一個峰值 —— 不要沿用 PP-OCRv5 時代「目標約 30px」的說法。
